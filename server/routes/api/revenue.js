...
async function getBookingPaidPayments(dateFrom, dateTo) {
...
router.get("/most-booked-fields", async (req, res) => {
  try {
    ...
    const payments = await getBookingPaidPayments(dateFrom, dateTo); // tìm tất cả thanh toán trong khoảng ngày
...
router.get("/field-area-income", async (req, res) => {
  try {
    ...
    // trả về mảng có tên field tương ứng
    const fieldNumber = await Promise.all(
      Object.entries(fieldOccurrence)?.map(async (field) => {
        const fieldDetail = await Field.findById(field[0]); // Tìm field detail theo id
        return {
          fieldId: field[0],
